...
process_kpagetable_init()
{
	...
	// mappages(kpagetable, CLINT, 0x10000, , PTE_R | PTE_W);
...
void procinithart(pagetable_t pagetable)
{
...
int uvmcopypt(pagetable_t pagetable, pagetable_t kpagetable, uint64 sz, uint64 start)
{
	pte_t *pte;
	uint64 pa, i;
	uint flags;
	...
	for (i = PGROUNDUP(start); i < sz; i += PGSIZE)
	{
		...
		{
			panic("uvmcopypt: pte does not exist.");
		}
		if ((*pte & PTE_V) == 0)
			panic("uvmcopypt: page not present");
		pa = PTE2PA(*pte);
